...
import type { ILayoutConfig, ILayoutState, IPrimaryColor, TColorScheme, TMenuMode, TMenuTheme } from '../layout/types'
...
export function useLayout() {
  ...
  //#region Datas
  const primaryColors = ref<IPrimaryColor[]>([
    { name: 'noir', palette: {} },
    {
      name: 'emerald',
      palette: {
  ...
  const applyTheme = (type: string, color: IPrimaryColor) =>{
    if (type === 'primary') {
  ...
  const updateColors = (type: string, color: IPrimaryColor) => {
    if (type === 'primary') {
  ...
  const updatePresetChange = (preset: string) => {
  ...
  const changeMenuMode = (mode: TMenuMode) => {
    layoutConfig.menuMode = mode
  }
  const changeMenuTheme = (theme: TMenuTheme) => {
    layoutConfig.menuTheme = theme
  }
  const getColorScheme = async (colorSchemeMode: TColorScheme) => {
    let colorScheme;
  ...
  const convertThemeMode = (
    colorSchemeMode: TColorScheme,
    darkTheme: boolean,
  ) => {
    if (colorSchemeMode === 'auto') {
  ...
  const changeColorScheme = async (colorSchemeMode: TColorScheme) => {
    layoutConfig.colorScheme = colorSchemeMode;
    if (colorSchemeMode === 'auto' || colorSchemeMode === 'light') {
  ...
  //#region Computed
  const currentColorScheme = computed(() => layoutConfig.colorScheme);
  ...
  const isNotebookToDesktopMode = computed(
    () => windowWidth.value >= EBreakPoint.NotebookLG,
  );

  const isNotebookMode = computed(
    () =>
      windowWidth.value > EBreakPoint.TabletMD &&
      windowWidth.value <= EBreakPoint.NotebookLG,
  );

  const isTabletToPhoneMode = computed(
    () => windowWidth.value <= EBreakPoint.TabletMD,
  );

  const isTabletMode = computed(
    () =>
      windowWidth.value > EBreakPoint.PhoneSM &&
      windowWidth.value <= EBreakPoint.TabletMD,
  );

  const isPhoneMode = computed(() => {
    return windowWidth.value <= EBreakPoint.PhoneSM;
  });
